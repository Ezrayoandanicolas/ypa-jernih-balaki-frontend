...
export default {
    ...
    async mounted() {
        try {
            await this.ARetrievePrograms().then((res) => {
                if (res.data.length == 0) {
                    this.ProgramArticle = null
                } else {
                    this.ProgramArticle = res.data
                }
    ...
    methods: {
        ...mapActions({
            ARetrievePrograms: 'ARetrievePrograms',
        }),
        deleteProgram(data) {
            this.$swal.fire({
                title: 'Are you sure? ',
                text: "Delete Program "+data.title+"?",
                icon: 'warning',
                ...
                confirmButtonText: 'Yes, delete it!'
                    ...
                    if (result.isConfirmed) {
                        this.$store.dispatch('ADeletePrograms', data.id).then(() => {
                            this.$swal.fire({
                                text: 'Berhasil Delete Program!',
                                icon: 'success',
...
<template>
    <div class="admin-program-list">
        <div v-if="ProgramArticle == null" class="relative items-center block w-full p-6 bg-gray-100 border border-gray-100 rounded-lg shadow-md animate__animated animate__fadeIn">            
        ...
        <div v-else v-for="(item, index) in ProgramArticle" :key="index" class="content mt-5 animate__animated animate__fadeIn animate__faster">
            ...
            <div class="program-list">
                <div class="flex m-auto bg-gray-100 border border-gray-200 rounded-lg shadow-sm hover:shadow-lg flex-row w-2/3 hover:bg-gray-100">
                    ...
                    <div class="flex flex-col p-4 leading-normal">
                        <router-link :to="'program/read/'+item.slug">
                        ...
                        <div class="flex justify-start text-gray-700">
                            <div class="edit">
                                <router-link :to="'program/update/'+item.id">
                                    <PencilSquareIcon class="w-6 h-6 text-blue-500"/>
                                </router-link>
                            </div>
                            <div @click="deleteProgram(item)" class="delete"><TrashIcon class="w-6 h-6 text-red-500"/></div>
                        </div>
                    </div>
                </div>
            </div>
